...
- **Change** — modal for revised hours → writes `Changed` to the confirmer's column + `oldHours->newHours` note (e.g. `2->1.5`) to the **Note** column via `setHourVerificationChangedNote` (Note column auto-detected from the `Note` header in row 2, column AU / index 46 by default, overridable via `HOUR_VERIFICATION_NOTES_COLUMN`). **The `X->Y` numbers must be at the START of the Notes cell** — the sheet's formulas parse them from the front — so `setHourVerificationChangedNote` writes the numbers first and preserves any earlier cell content after them (` | ` separator). The bot does **not** write columns B or C — the `Changed` verdict + note drive the sheet's own formulas.
- **Deny** — modal for a reason → writes `Denied` to the confirmer's column + the reason text to the **Note** column (`setHourVerificationNote`, which overwrites the cell).

**Confirmer resolution** (`getApproversForConfirmer` in `utils/sheets.js`): column F (default) may hold a single name, a comma-separated list, or a group label. `isGroupConfirmerLabel` (the single shared check — don't duplicate its logic elsewhere) only matches `ec/bd` / `ec / bd` shorthand or `anyone` combined with `ec`/`bd` (e.g. `Anyone on EC/BD`, `Anyone on the EC`). A bare `Executive Committee` or `Board of Directors` string (no `anyone`/`ec/bd`) is treated as a literal name lookup, not a group label — this was previously a bug where those bare phrases triggered the group DM behavior. Group labels expand to **all** EC/BD contacts with a Discord ID (`getECBDContacts`). Results are deduplicated by Discord ID. EC/BD group-label approvers write to column C (overall Verdict) since they have no named confirmer column (`confirmerColumnIndex = null`). Named confirmer approvers write to their own column under the header built from rows 1–2.
...
### Random BD audit (`utils/hourAudit.js`)
After each finalized hour decision, `maybeTriggerHourAudit` (called from the three terminal branches in `hourApprovalSync.js`) rolls `Math.random() < HOUR_AUDIT_CHANCE` (NaN-checked default `0.025` — an explicit `0` disables, `1` always triggers; do NOT `||`-default it) and on a hit posts a BD review request to `NT_LEADERS_CHANNEL_ID` pinging `BD_ROLE_ID`. **Require direction is one-way**: `hourApprovalSync.js` → `hourAudit.js`, never the reverse (`parseHoursInput` is deliberately duplicated in `hourAudit.js` for this reason).

Session model: `client.hourAuditPending` is a **flat** `Map<rowNumber, session>` (one audit per row, unlike the nested approval map), persisted to `data/hour-audit-state.json` on every mutation and restored by `restoreHourAuditSessions` (called unconditionally from `events/ready.js`, even when `HOUR_APPROVAL_ENABLED` is off, so already-posted audit buttons stay actionable). Custom IDs: `hour_audit_claim_${rowNumber}`, `hour_audit_approve_/change_/deny_${rowNumber}` (the Change modal reuses `hour_audit_change_${rowNumber}`); routed as boolean-returning handlers right after the hour-approval handlers in both `InteractionCreate` listeners.

Claim gate is a **strict** `roles.cache.has(BD_ROLE_ID)` check (deliberately not `hasRequiredRole`, which auto-passes Administrators) plus a `user.id !== originalApproverId` self-review block; the claim mutates the session synchronously before any `await` to close the double-click race. `session.reviewerName` is the reviewer's **Leadership-sheet name** (matched by Discord ID via `getContacts()`, falling back to their Discord display name), so audit notes use the same names as the original approver's `approverSheetName`. `session.targetColumnIndex` is resolved at creation (per-approver column, else name-resolved via `resolveConfirmerColumnIndex`, else column C index 2) so a BD override writes to the **same cell** the original decision landed in — pass it straight to `setConfirmerHourStatus` with a null name so no re-resolution happens. Same-decision outcomes only append a note via `appendHourVerificationNote` (which preserves existing Note content with a ` | ` separator — unlike `setHourVerificationNote`, which overwrites); different-decision outcomes overwrite the verdict cell after a stale-cell guard (`getHourVerificationCellValue` must still equal the audited verdict). An override **to Changed** writes the Notes cell via `setHourVerificationChangedNote` so the new `X->Y` numbers lead the cell (sheet formulas parse from the front), with prior content and the `BD review:` annotation after them — never `appendHourVerificationNote` for changed hours.
